...
R = np.zeros(shape=(7, 7))

i=0
j=0

for word in read_words(r'F:\Dambe\test\mtDNA\realDisEdit15.dis'):
    if(is_number(word)):
        if(i==0):
            continue
        #print((word))
...
complete_encoded = imputer.train(train_epochs=10000, batch_size=7)

printed = ""
i = 0
j = 0

f = open(r'F:\Dambe\test\mtDNA\encoderEdited15forDambe.dis', "w+")
